...
**Judges context:** Claude Club members (AI enthusiasts who love Claude — they want to see Claude doing something real and impressive, not just a badge) + Amazon intern/SWE (will mentally validate Terraform — fake HCL is immediately spotted; `aws_iam_role`, `aws_lambda_permission`, `depends_on`, and variable blocks are the credibility signals).

**Audience-specific priorities:**
- Claude Club: Make Claude Sonnet 3.5 the *hero* of the page, not a footnote. Show the topology → prompt → HCL pipeline visually. The "Generated by Claude Sonnet 3.5 · 2.3s" annotation in the Terraform panel is the money shot.
- Amazon judge: HCL must include `aws_iam_role` with a real `assume_role_policy` JSON, `aws_iam_role_policy_attachment`, `aws_lambda_permission` (so API GW can invoke Lambda), `variable "db_password"`, and `depends_on` where appropriate. This is the difference between toy code and production code.
...
const PILLS = [
  { label: 'Claude Sonnet 3.5 API', accent: true },   // Claude Club: this is the hero
  { label: 'AWS Cloud Control API', accent: false },   // Amazon judge: real provisioning
